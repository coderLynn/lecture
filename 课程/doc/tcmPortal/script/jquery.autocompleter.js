...
;(function ($, window) {
     ...
     * @param customKey [boolean] <false> "The name is object's property which will be used as a search key just as a label. by gongjan"
            ...
            for (var i = 0; i < 2; i++) {
                for (var item in source) {
                    if (response.length < data.limit) {
                        ...
                        var key = (data.customKey && source[item][data.customKey]) ? source[item][data.customKey] : source[item].label;
                        if (escape(query).indexOf( "%u" )<0) label = key;//search key
                        switch (i) {
                        case 0:
    ...
    function _xhr(data) {
        if (typeof data.source === 'object') {
            ...
            data.jqxhr = $.ajax({
                ...
                crossDomain: true,
                data:       ajaxData,
                type:       'POST', // by gongjan maybe change not cache
    ...
    function _open(e, instanceData) {
        ...
        if (!data.$node.prop('disabled') && !data.$autocompleter.hasClass('autocompleter-show') && data.$list && data.$list.length) {
            ...
            //reset autocompleter div's width by gongjan
            data.$autocompleter.css("width",data.$node.css("width"));
        }
    }
    ...
    function _getCache(url, expires) {
        ...
        if (item && item.value && item.value.length > 0) { // item.value.length > 0 add by gongjan.  
            response = item.value;
